...
export default function RequestsPage() {
  ...
  const changeStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: Request['status'] }) =>
      requestsApi.changeStatus(id, status),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Запросы</h1>
          <p className="text-gray-600">Управление запросами клиентов</p>
        </div>
        <button
          onClick={() => {/* TODO: Implement create request */}}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">
              {search || statusFilter ? 'Попробуйте изменить фильтры' : 'Создайте первый запрос'}
            </p>
          </div>
        ) : (
          <>
                  ...
                  {data?.items.map((request) => (
                    <tr key={request.id} className="table-row">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                        {request.client?.full_name || 'Не указан'}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                        {request.assigned_user?.full_name || 'Не назначен'}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-right">
                        <div className="relative">
                          <button
                            onClick={() => setMenuOpenId(menuOpenId === request.id ? null : request.id)}
