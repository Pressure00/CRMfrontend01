...
export default function EditDeclarationPage() {
  ...
  useEffect(() => {
    const fetchDeclaration = async () => {
      try {
        const response = await declarationsApi.getById(parseInt(id!));
        const declaration = response.data;
        reset({
          client_name: declaration.client_name || '',
          client_phone: declaration.client_phone || '',
          client_email: declaration.client_email || '',
          declaration_date: declaration.declaration_date,
          deadline: declaration.deadline || '',
          priority: declaration.priority,
          notes: declaration.notes || '',
        });
        setVehicles(declaration.vehicles || []);
      } catch (error: any) {
    ...
    if (id) fetchDeclaration();
  }, [id, reset, navigate]);

  const onSubmit = async (data: DeclarationForm) => {
    try {
      await declarationsApi.update(parseInt(id!), {
        ...data,
        deadline: data.deadline || null,
        client_email: data.client_email || null,
      });
      toast.success('Декларация обновлена');
      navigate(`/declarations/${id}`);
    } catch (error: any) {
      toast.error(error.response?.data?.detail || 'Ошибка обновления декларации');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-4">
        <button onClick={() => navigate(`/declarations/${id}`)} className="btn-outline">
          <ArrowLeft className="h-4 w-4 mr-2" />
          Назад
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Редактировать декларацию</h1>
          <p className="text-gray-600">Измените информацию о декларации</p>
        </div>
      </div>
